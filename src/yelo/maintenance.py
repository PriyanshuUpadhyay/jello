...
def checkout():
    ...
    if not (root / "pyproject.toml").is_file() or not (root / ".git").exists():
        raise MaintenanceError("this command needs an editable install from a Git checkout; "
                               "a Homebrew install updates with: brew upgrade yelo")
    return root
...
def update(root, *, check=False, with_hud=False):
    ...
    uv = shutil.which("uv")
    if not uv:
        raise MaintenanceError("uv is required to update Yelo")
    launcher = hud.launcher_path()
    home = os.path.expanduser("~")
    if with_hud and not os.path.isfile(hud.binary_path(home)):
        raise MaintenanceError("HUD is not installed; run yelo hud install first")
    print(f"Source: {root}\nUpstream: {upstream}", flush=True)
    if check:
        print("Ready to fast-forward this branch, reinstall Yelo, and apply setup.")
        if with_hud:
...
def release(root, output, tag=None):
    ...
    if metadata["project"]["name"] != "yelo" or not re.fullmatch(r"[0-9]+\.[0-9]+\.[0-9]+", version):
        raise MaintenanceError("release requires a Yelo version in MAJOR.MINOR.PATCH form")
    module = git(root, "show", "HEAD:src/yelo/__init__.py")
    declared = re.search(r'''^__version__\s*=\s*(["'])([0-9.]+)\1\s*$''', module, re.MULTILINE)
    if not declared or declared[2] != version:
        raise MaintenanceError("pyproject.toml and src/yelo/__init__.py versions must match")
    if tag is not None:
    ...
    with tempfile.TemporaryDirectory(prefix="yelo-release-") as staged:
...
def register(subparsers):
    parser = subparsers.add_parser("update", help="update this Yelo checkout and apply setup")
